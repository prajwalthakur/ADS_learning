...
def show_pcl(pcl):
    ...
    # Assuming 'pcl' is your original numpy array with 4 columns 
    pcd.points = o3d.utility.Vector3dVector(pcl[:, :3]) # Extract only the first three columns (x, y, z)
    
    # step 4 : for the first frame, add the pcd instance to visualization using add_geometry; for all other frames, use update_geometry instead
    vis.add_geometry(pcd)

    def key_callback(vis):
        # This function will handle key events; in this case, updating the geometry with new data if necessary
        # Here you could handle updating the pcd with new data points
        vis.update_geometry(pcd)
        vis.poll_events()
        vis.update_renderer()

    # Register the callback for right-arrow key (key-code 262)
    vis.register_key_callback(262, key_callback)

    # step 5 : visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    vis.run()  # Start the visualization loop
    vis.destroy_window()  # Close the window when done
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    # _, inverse_indices = np.unique(lidar_pcl_top[:, 0:2], axis=0, return_inverse=True)
